...
def plot_signals(signals, labels):
    ...
    alphas = [
        ...
        0.45,
        0.45,
    ]  # just some opacity values to facilitate visualization
    ...
    for j, sig in enumerate(signals):  # iterates on all signals
        plt.plot(
            range(lenght), sig, "-o", label=labels[j], markersize=2, alpha=alphas[j]
        )
    plt.grid()
    plt.ylabel("RSSI")
    plt.xlabel("time")
...
def plot_distance_to_rssi_correlation(subdirectory: str):
    ...
    y = []
    filenames = glob.glob(
        f"./data/test0_rssi_to_distance_correlation/{subdirectory}/*_beacon22.csv"
    )
    ...
    plt.plot(
        ...
        y_real,
        "yo-",
        label="значение по формуле rssi=-10*N*log10(distance)+RSSI_AT_1M",
    )
...
def plot_rssi_to_time(subdirectory: str):
    ...
    timestamp: float = 0
    filename = glob.glob(
        f"./data/test0_rssi_to_distance_correlation/{subdirectory}/*_3m.csv"
    )[0]
    with open(filename, "r") as file:
        reader = csv.DictReader(file)
...
def plot_occurrence_frequency(subdirectory: str):
    filenames = glob.glob(
        f"./data/test0_rssi_to_distance_correlation/{subdirectory}/*.csv"
    )
    filenames.sort()
    ...
    for index, ax in enumerate(axes.flatten()):
        ...
        if m == 1:
            count = (
                np.count_nonzero(data < 60)
                + np.count_nonzero(np.logical_and(data > -56, data < -53))
                + np.count_nonzero(data > -49)
            )
        if m == 2:
            count = np.count_nonzero(data < -48) + np.count_nonzero(data > -44)
        if m == 3:
            count = (
                np.count_nonzero(data < -72)
                + np.count_nonzero(np.logical_and(data > -68, data < -48))
                + np.count_nonzero(data > -59)
            )
        if m == 4:
            count = (
                np.count_nonzero(data < -82)
                + np.count_nonzero(np.logical_and(data > -77, data < -74))
                + np.count_nonzero(data > 68)
            )
...
def plot_rssi_for_beacon(signal):
    ...
    signal_particle_filter = particle_filter(
        signal, quant_particles=100, A=1, H=1, Q=1.6, R=6
    )
    plot_signals(
        [
            signal,
            ...
            signal_particle_filter,
        ],
        [
            "signal",
            "gray_filtered_signal",
            "fft_filtered_signal",
            "kalman_filtered_signal",
...
def plot_occurrence_frequencies_for_different_beacons():
    filenames = glob.glob(
        f"./data/test0_rssi_to_distance_correlation/balcony/3000_samples_3m*.csv"
    )
